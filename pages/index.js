...
export default function AdminPage() {
  ...
  const addWish = async () => {
    ...
    setTitle(""); setDescription(""); setCategory(""); setNewCategory("");
    loadWishes();
  };
  ...
  const handleMarkTaken = async (wish) => {
    ...
    await fetch("/api/wishes", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: wish._id, taken: true, takenBy, quantity: Number(quantity) }),
    });
    loadWishes();
  };
  ...
  const confirmMarkUntaken = async () => {
    await fetch("/api/wishes", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: wishToMarkUntaken._id, taken: false, takenBy: "", quantity: 0 }),
    });
    setConfirmModalOpen(false); setWishToMarkUntaken(null);
    loadWishes();
  };

  const openDeleteModal = (wish) => { setWishToDelete(wish); setDeleteModalOpen(true); };
  const deleteWish = async () => {
    await fetch("/api/wishes", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: wishToDelete._id }) });
    setDeleteModalOpen(false); setWishToDelete(null); loadWishes();
  };
  ...
  const saveEdit = async () => {
    ...
    setEditModalOpen(false); setWishToEdit(null);
    loadWishes();
  };
  ...
  const groupWishesByCategory = () => {
    ...
    return Object.keys(grouped).sort().map(cat => ({ category: cat, wishes: grouped[cat] }));
  };
  ...
  if (!session) {
    return (
      <div style={{ maxWidth: 350, margin: "100px auto", padding: 20, textAlign: "center", fontFamily: "Arial", background: "#fff", borderRadius: 10, boxShadow: "0 0 20px rgba(0,0,0,0.1)" }}>
        <h2>Admin Login</h2>
        <button style={{ padding: "10px 15px", width: "100%", background: "#0070f3", color: "white", border: "none", borderRadius: 5, cursor: "pointer" }} onClick={() => signIn("credentials")}>
          Sign in
        </button>
      </div>
    );
    ...
    <div style={{ maxWidth: 700, margin: "40px auto", fontFamily: "Arial, sans-serif", padding: 20, background: "#f3c1c1ff", borderRadius: 10, boxShadow: "0 0 20px rgba(0,0,0,0.1)" }}>
      ...
      {groupWishesByCategory().map(({ category, wishes }) => (
        <div key={category}>
          ...
          {wishes.map(wish => (
            <div key={wish._id} style={{ margin: "10px 0", padding: 15, borderRadius: 8, background: wish.taken ? "#ffe6e6" : "#fff", borderLeft: wish.taken ? "5px solid red" : "5px solid #0070f3", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
      ...
      {confirmModalOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ background: "#fff", padding: 20, borderRadius: 8, minWidth: 300, textAlign: "center" }}>
            <p>Mark <strong>{wishToMarkUntaken.title}</strong> as untaken?</p>
            <button style={{ margin: 10, padding: "10px 15px", background: "#0070f3", color: "white", border: "none", borderRadius: 5 }} onClick={confirmMarkUntaken}>Yes</button>
            <button style={{ margin: 10, padding: "10px 15px", background: "#0070f3", color: "white", border: "none", borderRadius: 5 }} onClick={() => setConfirmModalOpen(false)}>Cancel</button>
          </div>
        </div>
      )}
...
{editModalOpen && (
  ...
  >
    ...
    >
      <h2 style={{ textAlign: "center", marginBottom: 20 }}>Edit Wish</h2>

      <div style={{ marginBottom: 15 }}>
        <label style={{ fontWeight: "bold", display: "block", marginBottom: 5 }}>Title</label>
        <input
          type="text"
          value={editTitle}
      ...
      <div style={{ marginBottom: 15 }}>
        ...
        <textarea
          ...
          style={{ width: "100%", padding: 8, borderRadius: 5, border: "1px solid #ccc", minHeight: 60 }}
        />
      </div>
  ...
  <div style={{ display: "flex", alignItems: "center" }}>
    ...
    <button
      type="button"
      onClick={() => setEditQuantity(prev => Math.max(0, Number(prev) - 1))}
      style={{
        padding: "6px 10px",
        marginRight: 5,
        background: "#ccc",
    ...
    <button
      type="button"
      onClick={() => setEditQuantity(prev => Number(prev) + 1)}
      style={{
        padding: "5px 10px",
        marginLeft: 5,
        background: "#ccc",
    ...
    >
      ...
      <div style={{ marginBottom: 15 }}>
        <label style={{ fontWeight: "bold", display: "block", marginBottom: 5 }}>Category</label>
        <select
          value={editCategory}
      ...
      <div style={{ marginBottom: 15 }}>
        ...
        {editTaken && (
          <div style={{ marginTop: 10 }}>
            <label style={{ fontWeight: "bold", display: "block", marginBottom: 5 }}>Taken By</label>
            <input
              type="text"
              value={editTakenBy}
